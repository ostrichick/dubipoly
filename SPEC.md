...
## UI and architecture
Reference is guidance, never a screenshot with overlaid buttons. Functional cells and detail panel; landscape side panel, portrait scrollable board and panel below. The current Dubu photo is temporary artwork; replace it with a character illustration when the image-generation asset is available. React/TypeScript with Vite/Vinext from Sites scaffold, shadcn Button, CSS. Data in lib/board.ts; the rule engine is separate from UI. The hosted PWA currently supports owner-private two-player rooms.
